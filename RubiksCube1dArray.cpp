...
class RubiksCube1dArray: public RubiksCube{
    ...
    void rotateFace(int ind){
        ...
        for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
                temp[3*i+j]=cube[getIndex(ind,i,j)];
            }
        }


        for(int i=0;i<3;i++)cube[getIndex(ind,0,1)]=temp[getIndex(0,2-i,0)];
        for(int i=0;i<3;i++)cube[getIndex(ind,i,2)]=temp[getIndex(0,0,2-i)];
        for(int i=0;i<3;i++)cube[getIndex(ind,2,2-i)]=temp[getIndex(0,i,2)];
        for(int i=0;i<3;i++)cube[getIndex(ind,2-i,0)]=temp[getIndex(0,2-i,0)];
    }
